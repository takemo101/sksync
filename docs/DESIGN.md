...
lockfile は `package-lock.json` と同じく、別環境で `sksync install` した時に同じ skill body を再構成できる情報だけを保持する。対応 OS は当面 macOS / Linux とし、Windows 固有の path / symlink 差分は対象外にする。Linux 配布は distro 間の glibc 差分を避けるため musl release asset を基本にする。
...
Release workflow は macOS target に加えて Linux musl target を生成する。
...
Linux installer は `uname -s` / `uname -m` から asset target を選ぶ。Docker smoke workflow は x86_64 musl binary を Debian / Ubuntu containers で実行し、local source に対する `init` / `add` / `plan` / `apply` / `check` / `list` / `remove` を検証する。
...
Windows は引き続き対象外。Alpine は musl binary で動く可能性が高いが、正式 smoke coverage にはまだ含めない。
...
| `sksync remove <skills...>`             | `npm uninstall`                 | 1件以上の dependency / installed skill / lockfile entry / managed symlink を削除する      |
...
### `sksync remove <skills...>`

- 1件以上の skill name を受け取れる
...
- 既存ファイルと衝突しないか検査する
- symlink を作成・更新する
- `--force` は sksync-managed link と判断できる target の安全な置換だけを許可する
- lockfile を生成する
...
- ユーザーに「追加 / 削除 / agent 変更 / default agents 設定 / 状態確認」などの intent を選ばせる
...
- `defaultAgents` のような wizard preference は既存 config fields を保持した JSON 更新に限定する
...
- `Add skill` の agent prompt は config の `defaultAgents` を初期選択に使い、ユーザーが毎回変更できる
