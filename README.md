...
cargo run -- add owner/repo/path/to/skill#version --provider skills.sh --agent pi
cargo run -- add skills.sh/owner/repo/path/to/skill#version --agent pi
...
lockfile と upstream を比較して、更新可能な skill を表示します。Git source は remote ref の HEAD と lockfile の resolved commit を比較します。Registry source は `skills.sh` を除き provider 未実装の場合 `registry-provider-missing` として表示します。
...
- Additional registry providers beyond `skills.sh`
- GitLab / gist support
